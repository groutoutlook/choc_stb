...
[[nodiscard]] inline std::string removeDoubleQuotes(std::string text)
{
	return removeOuterCharacter(std::move(text), '"');
}
[[nodiscard]] inline std::string removeSingleQuotes(std::string text)
{
	return removeOuterCharacter(std::move(text), '\'');
}

[[nodiscard]] inline std::string addDoubleQuotes(std::string text)
{
	return "\"" + std::move(text) + "\"";
}
[[nodiscard]] inline std::string addSingleQuotes(std::string text)
{
	return "'" + std::move(text) + "'";
}
...
{
	auto d1 = c - static_cast<uint32_t>('0');
	if (d1 < 10u)
...
inline std::string_view trimStart(const char *text)
{
	return trimStart(std::string_view(text));
}
inline std::string_view trimEnd(const char *text)
{
	return trimEnd(std::string_view(text));
}
...
{
	return splitString(text, [](char c) { return isWhitespace(c); }, [](char c) { return isWhitespace(c); }, keepDelimiters);
}
...
inline bool contains(std::string_view t, std::string_view s)
{
	return t.find(s) != std::string::npos;
}
inline bool startsWith(std::string_view t, char s)
{
	return !t.empty() && t.front() == s;
}
inline bool endsWith(std::string_view t, char s)
{
	return !t.empty() && t.back() == s;
}
...
inline std::string getByteSizeDescription(uint64_t size)
{
	...
	if (size >= 0x40000000)
		return intToStringWith1DecPlace(std::min(maxValue, size), 0x40000000) + " GB";
	if (size >= 0x100000)
		return intToStringWith1DecPlace(size, 0x100000) + " MB";
	if (size >= 0x400)
		return intToStringWith1DecPlace(size, 0x400) + " KB";
	if (size != 1)
